...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    appendColumnButton.addEventListener("click", ()=>{
        if (table.rows[0].cells.length < MAX_COUNT) for (const row of table.rows)row.insertCell();
        updateButtonState();
    });
    removeColumnButton.addEventListener("click", ()=>{
        if (table.rows[0].cells.length > MIN_COUNT) for (const row of table.rows)row.deleteCell(-1);
        updateButtonState();
    });
    updateButtonState();
});
